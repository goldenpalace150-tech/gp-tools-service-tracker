...
GP_AR_STARTS = [
    "يا أبطال القصر الذهبي", "يعطيكم العافية يا شباب", "يا جماعة الورشة",
    "يا فريقنا الذهبي", "أهل الهمة بالورشة", "شباب القصر الذهبي",
]
GP_AR_MOTIVATION = [
    "كل حالة منسكرها صح بتزيد ثقة الزبون فينا",
    "الشغل المرتب اليوم بيريحنا بكرا",
    "كل جهاز عم يطلع مضبوط هو نقطة إلنا",
    "السرعة حلوة بس الدقة أحلى",
    "المتابعة الواضحة بتختصر نص الشغل",
    "ما منترك حالة معلقة بلا سبب واضح",
    "كل خطوة صغيرة اليوم بتعمل فرق كبير بنهاية الدوام",
    "خبرتنا بتبين بالتفاصيل مو بالحكي",
    "الزبون ما بيشوف كل التعب اللي ورا الكواليس، بس بيحس بالنتيجة المرتبة",
    "لما كل واحد يحدث حالته بوقتها، الفريق كله بيصير أسرع",
]
GP_AR_ACTION = [
    "خلونا نركز على الحالات الأقدم أول",
    "يلا منقفل المفتوح حالة ورا حالة",
    "اللي جاهز ما نخليه يستنى",
    "كل واحد يحدث حالته أول بأول",
    "خلونا نخلي القائمة أخف قبل نهاية الدوام",
    "إذا في عائق نرفعه بسرعة وما نخليه ينام",
    "الأولوية اليوم للحالة اللي مستنية قرار واضح",
    "نرتب الشغل ونخلي كل جهاز بمكانه الصحيح",
    "الحالة اللي ما إلها خطوة جاية واضحة بدها قرار اليوم",
    "نخلي المعلومة تمشي أسرع من الجهاز بين الأقسام",
]
GP_AR_FUNNY = [
    "اليوم الأجهزة داخلة الورشة متوترة، طمنوها إنو وصلت لعند أهل الخبرة",
    "إذا جهاز عند معنا، ما في مشكلة، منعنّد عليه بالخبرة أكتر",
    "قائمة الانتظار عم تعمل حالها كبيرة، بس نحنا منعرف كيف نصغرها",
    "الجهاز اللي مفكر يقضي عطلة عنا، خبرّوه إنو الحجز فل اليوم",
    "إذا مفك البراغي اختفى مرة تانية، رح نعمله سند صيانة لحالو",
    "الورشة اليوم بدها شغل مرتب وابتسامة، لأن الأجهزة كمان إلها نفسية",
    "أي جهاز عم يمثل إنو ما فيه عطل، عنا فنيين بيعرفوا يخلّوه يعترف",
    "خلونا نشتغل بهدوء، بس بسرعة تخلي القائمة نفسها تستغرب",
    "إذا الشاشة صارت كلها أخضر، لا حدا يعمل ريفريش من الخوف، هاد اسمه إنجاز",
    "الداشبورد ما بيصلح الأجهزة، بس بيحرج الحالة اللي قاعدة بلا تحديث",
]
GP_AR_PERSON = [
    "{name}، اليوم عليك حركة حلوة بالقائمة، بدنا نشوف كم حالة رح تختفي",
    "{name}، الأجهزة عاملة موعد معك اليوم، لا تخليها تنتظر كتير",
    "{name}، ورجينا لمسة الخبرة وخلي آخر كلمة بالحالة تم",
    "{name}، شد الهمة، كل حالة بتخلصها عم تفتح مجال للي بعدها",
    "{name}، إذا الجهاز ما حكى من أول فحص، اسأله بالطريقة الفنية تبعك",
    "{name}، تحديث صغير منك هلق بيوفر ثلاث أسئلة بعد شوي",
]
GP_AR_MANAGER = [
    "توجيه من أبو آدم: الأولوية للحالات المتأخرة والعالقة، وأي عائق واضح ارفعوه بسرعة مشان ما يوقف حركة الشغل",
    "أبو آدم متابع حركة العمليات اليوم، خلوا كل حالة محدثة والخطوة الجاية واضحة حتى كل فريق يعرف شو عليه",
    ...
    "أبو آدم عم ينسق حركة الشغل بين الأقسام، خلوا الأولويات واضحة والمعلومة توصل بسرعة وما نخلي أي حالة تضيع بين فريق وفريق",
]
GP_AR_MANAGER_JOKES = [
    "أبو آدم طلب تطوير بسيط، ومن خبرتنا بالتطوير البسيط، إذا خلصناه اليوم بكرا بيطلعله إصدار رقم اثنين",
    "أبو آدم بيقول إذا الداشبورد صار أخضر كله لا تخافوا، مو عطل بالنظام، هاد إنجاز حقيقي",
    "أبو آدم طلب نشيل البوتل نِك، بس رجاء لا حدا ينقله من قسم لقسم ويسميها إعادة توزيع",
    "رسالة تطوير من أبو آدم: أي اختصار بخطوات الشغل مرحب فيه، إلا إذا الاختصار رجعنا ثلاث خطوات لورا",
    "أبو آدم عم يطور العمليات لدرجة إذا العملية زبطت زيادة، غالباً رح نفتح مشروع تطوير جديد لنعرف ليش زبطت",
    "إذا لقينا عملية بدها خمس موافقات مشان تصير أسرع، خبروا أبو آدم قبل ما نضيف الموافقة السادسة",
]
GP_AR_ENDS = [
    "والله يعطيكم العافية", "وبالتوفيق يا شباب", "يلا نكملها صح",
    "والقهوة بعد الإنجاز أطيب", "وخلي الزبون يحس بالفرق", "ومنكمّل بنفس النفس الحلو",
]

GP_EN_STARTS = ["Golden Palace team", "Workshop heroes", "Great team", "Golden Palace crew", "Service team"]
GP_EN_MOTIVATION = [
    "every properly closed case builds more customer trust",
    "clean follow-up today saves time tomorrow",
    "speed matters, but accuracy matters more",
    "clear updates make the whole workshop faster",
    "our experience shows in the details",
]
GP_EN_ACTION = [
    "let us clear the oldest cases first", "keep every case updated as you work",
    "raise blockers early instead of letting them sleep", "keep the queue moving",
    "make the next action clear on every case",
]
GP_EN_FUNNY = [
    "the waiting list is trying to look big again, let us disappoint it",
    "if the screwdriver disappears again, we may open a service ticket for it",
    "if the dashboard turns completely green, do not panic, that is called progress",
]
GP_EN_MANAGER = [
    "A note from Abu Adam: prioritize delayed and blocked cases, and raise obstacles early so operations keep moving",
    "Abu Adam is following today’s operations; keep every case updated and make the next action clear",
    "Abu Adam reminds the team that good operations means clear ownership, fewer blockers, and less customer waiting time",
]
GP_EN_MANAGER_JOKES = [
    "Abu Adam asked for one small improvement; operations experience says version two is already waiting around the corner",
    "Abu Adam says if the dashboard turns completely green, do not restart it; that is called an achievement",
    "Abu Adam asked us to remove the bottleneck, not move it to another department and call it redistribution",
]
...
def gp_build_voice_message(lang="ar"):
    ...
    if lang == "ar":
        if r < 0.12:
            category = "management_joke"
            person = "أبو آدم"
            text = f"{random.choice(GP_AR_MANAGER_JOKES)}. {random.choice(GP_AR_ENDS)}."
            tone = "funny"
        elif r < 0.27:
            category = "management"
            person = "أبو آدم"
            text = f"{random.choice(GP_AR_MANAGER)}. {random.choice(GP_AR_ENDS)}."
        elif r < 0.48:
            category = "funny"
            text = f"{random.choice(GP_AR_STARTS)}، {random.choice(GP_AR_FUNNY)}. {random.choice(GP_AR_ENDS)}."
            tone = "funny"
        elif r < 0.66:
            category = "person"
            person = random.choice(GP_STAFF_AR)
            text = f"{random.choice(GP_AR_PERSON).format(name=person)}. {random.choice(GP_AR_ENDS)}."
        elif r < 0.86:
            text = f"{random.choice(GP_AR_STARTS)}، {random.choice(GP_AR_MOTIVATION)}، {random.choice(GP_AR_ACTION)}. {random.choice(GP_AR_ENDS)}."
        else:
            category = "workshop"
            text = f"{random.choice(GP_AR_STARTS)}، {random.choice(GP_AR_ACTION)}. {random.choice(GP_AR_MOTIVATION)}. {random.choice(GP_AR_ENDS)}."
    else:
        if r < 0.12:
            category = "management_joke"
            person = "Abu Adam"
            text = f"{random.choice(GP_EN_MANAGER_JOKES)}. {random.choice(GP_EN_ENDS)}."
            tone = "funny"
        elif r < 0.27:
            category = "management"
            person = "Abu Adam"
            text = f"{random.choice(GP_EN_MANAGER)}. {random.choice(GP_EN_ENDS)}."
        elif r < 0.48:
            category = "funny"
            text = f"{random.choice(GP_EN_STARTS)}, {random.choice(GP_EN_FUNNY)}. {random.choice(GP_EN_ENDS)}."
            tone = "funny"
        elif r < 0.66:
            category = "person"
            person = random.choice(GP_STAFF_EN)
            text = f"{person}, keep the case moving and make the next update clear. {random.choice(GP_EN_ENDS)}."
        else:
...
def gp_ensure_voice_file(text, voice, rate, pitch):
    ...
    with GP_VOICE_FILE_LOCKS_GUARD:
        lock = GP_VOICE_FILE_LOCKS.setdefault(token, threading.Lock())
    with lock:
        if os.path.exists(path) and os.path.getsize(path) > 1000:
            return token, path
        tmp_path = path + ".tmp"
        try:
            ...
            if not os.path.exists(tmp_path) or os.path.getsize(tmp_path) <= 1000:
                raise RuntimeError("voice generation returned an empty audio file")
...
def api_voice_track():
    ...
    return jsonify({
        ...
        "person": track["person"],
        "voice": voice,
        "audio_url": f"/api/voice-audio/{token}.mp3",
    })
...
if 'GP_SERVER_VOICE_CLIENT_V2' not in tv:
    # Remove browser-specific labels from both languages.
    ...
    // Audio is synthesized on the Flask server; browser/OS voice engines are never used.
    ...
    function gpPrefetchServerVoice(){
        if(!gpServerVoicePrefetch){
            gpServerVoicePrefetch = gpFetchServerVoiceTrack().catch(() => null);
        }
        return gpServerVoicePrefetch;
    }
    ...
    async function playTeamVoice(_ignoredTrack=null, manual=false){
        ...
        let track = null;
        try{
            track = await (gpServerVoicePrefetch || gpFetchServerVoiceTrack());
        }catch(_){ track = null; }
        gpServerVoicePrefetch = null;
        gpPrefetchServerVoice();
    ...
    function scheduleNextTeamVoice(delay=TEAM_VOICE_INTERVAL_MS){
        ...
        teamVoiceTimer = null;
        if(detailMode || !teamVoiceEnabled) return;
        teamVoiceTimer = setTimeout(async () => {
            try{
                if(!teamVoiceEnabled || document.hidden || teamVoicePlaying){
                    scheduleNextTeamVoice(TEAM_VOICE_RETRY_MS); return;
                }
                const ok = await playTeamVoice(null, false);
                scheduleNextTeamVoice(ok ? TEAM_VOICE_INTERVAL_MS : TEAM_VOICE_RETRY_MS);
            }catch(_){ scheduleNextTeamVoice(TEAM_VOICE_RETRY_MS); }
        }, Math.max(1000, delay));
    }
    ...
    async function toggleTeamVoice(){
        ...
        try{
            ...
            localStorage.setItem('gpFemaleVoiceEnabled', teamVoiceEnabled ? '1' : '0');
            if(!teamVoiceEnabled){
                clearTimeout(teamVoiceTimer);
                stopTeamVoiceAudio();
                updateVoiceButton();
                showToast(tr('voiceDisabled'));
                return;
            }
            updateVoiceButton();
            gpPrefetchServerVoice();
        ...
        }catch(_){ updateVoiceButton('blocked'); }
    }
    ...
    async function testTeamVoice(){
        ...
        }catch(_){ showToast(tr('audioBlocked'), 7000); }
    }

    function initialiseTeamVoice(){
        if(detailMode) return;
        try{ teamVoiceEnabled = localStorage.getItem('gpFemaleVoiceEnabled') !== '0'; }
        catch(_){ teamVoiceEnabled = true; }
        updateVoiceButton();
        gpPrefetchServerVoice();
        // Start quickly after launch so the team immediately knows voice is working.
...
# ------------------------------------------------------------------
# Browser-neutral Windows kiosk launcher: Chrome OR Edge.
# ------------------------------------------------------------------
launcher = r'''@echo off
setlocal
...
if exist "%CHROME%" (
  start "Golden Palace TV" "%CHROME%" --user-data-dir="%PROFILE%-Chrome" --kiosk "%TVURL%" --no-first-run --disable-session-crashed-bubble --autoplay-policy=no-user-gesture-required
  exit /b 0
)
if exist "%EDGE%" (
  start "Golden Palace TV" "%EDGE%" --user-data-dir="%PROFILE%-Edge" --kiosk "%TVURL%" --edge-kiosk-type=fullscreen --no-first-run --disable-session-crashed-bubble --autoplay-policy=no-user-gesture-required
  exit /b 0
)
